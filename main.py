...
Usage:
    command count:                  number of actions to take on the list 1-100
         ...
         <number>:                  enter a command from the below methods format:
                                    ...
                                    If interective mode is chosen commands may be
                                    entered one at a time, after which the array 
                                    and any returns will be printed to the console

Available methods:    
    append(new_element)             add new_element to end of list
    clear()                         remove all elements from the list
    count(element)                  returns the number of occurances of element in
                                        list
    ...
    pop(<index>)                    remove an element from the list and return its
                                        value, if index is not provided the last 
    ...
    sort(key, reverse=True|False)   sort the list according to function key with 
                                        largest or smallest value first
...
def welcome():
    ...
    available in python 3. Originally it was created to solve a simple challenge 
    on HackerRank. Then it sorta took over if you need anything, feel free to 
    drop me a message
    '''
    print(greetings)
